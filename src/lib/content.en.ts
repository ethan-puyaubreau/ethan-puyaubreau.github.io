 ...
 *
 * Two case studies below (Opération Endgame, the community directory) are
 * deliberately domain-neutral: no game, platform, or community name, no
 * links. That is not an oversight. Both projects are real and both are tied,
 * elsewhere, to a separate identity this site does not name; a reader who
 * searched either title verbatim would find that identity in one query. The
 * shape of the work (an annual 120+-person live operation; a 57-entry
 * community directory, built and self-hosted) is the part worth showing.
 */
