...
- **`/cluster`**: the numbers are a build-time snapshot of the homelab's own telemetry
  endpoint (`src/data/cluster-snapshot.json`), not a live poll. That endpoint lives on a
  different domain (a separate, pseudonymous project of the author's); polling it at runtime
  from this site would be a standing cross-origin request between the two. To refresh the
  snapshot, regenerate that file by hand from the endpoint and rebuild.
- **No analytics**: deliberately. The self-hosted analytics instance used elsewhere lives on
  that same other domain.
